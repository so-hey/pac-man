...
// Capricious type. Aims at the point-symmetrical position of Shadow Blinky (red) in the shortest possible distance, with Pacman at the centre of the action.
const bashfulInkyAI = (
  ghostPos: { y: number; x: number },
  gameBoard: GameBoard,
  pacManPos: { y: number; x: number },
  ...
  // Check if ty and tx point to a legal cell, and adjust them if not
  if (
    ty < 0 ||
    ty >= gameBoard.length ||
    tx < 0 ||
    tx >= gameBoard[0].length ||
    gameBoard[ty][tx] === Cell.Wall
  ) {
    let minDist = Infinity;
    for (let y = 0; y < gameBoard.length; y++) {
      for (let x = 0; x < gameBoard[0].length; x++) {
        if (gameBoard[y][x] !== Cell.Wall) {
          const dist = Math.abs(y - ty) + Math.abs(x - tx);
          if (dist < minDist) {
            minDist = dist;
            ty = y;
            tx = x;
